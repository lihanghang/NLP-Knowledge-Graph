...
class Model(object):
    def __init__(self, config, is_train=True):
        ...
        # reduction_indices default None：0-dim
        length = tf.reduce_sum(used, reduction_indices=1)
        self.lengths = tf.cast(length, tf.int32)
        self.batch_size = tf.shape(self.char_inputs)[0]
        self.num_steps = tf.shape(self.char_inputs)[-1]

        # Add model type by Mason,Bi-LSTM or IDCNN
        ...
        # apply gradient clipping to avoid gradient explosion and gradient vanish 419 528
    ...
    def IDCNN_layer(self, model_inputs, name=None):
        ...
        with tf.variable_scope("idcnn" if not name else name):
            ...
            # squeeze：All dimensions of size 1 removed
